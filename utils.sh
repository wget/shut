...
## argument value. i.e.: getArgumentValue "--file|-f" $@
## @param $string The argument name in long, short format or both. If both are
## specified, this needs to be separated by a pipe (|).
## @param $array The list of the script arguments to check for.
## @return In $retval, the value of the argument, empty if the argument is not
## valid/existent.
## @retval 0 If the argument was found.
## @retval 1 If the argument was not found, or if an issue occurred.
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## @fn trim()
## @details Remove from the beginning or the end of string the characters
## defined in the second argument (regex supported)
...
## @param $mask The caracters to trim either as a simple string or a regex.
## @return In $retval, the trimmed string.
## @retval 0 Always, as up to now, we do not check if the regex has error in it.
#-------------------------------------------------------------------------------
# function trim() {
#     unset retval
#
...
#     fi
#
#     local regexPos=$((${#regex}-1))
#     if [[ "${regex:$regexPos:1}" == "$" ]]; then
#         regex=${regex:0:regexPos}'(.*)$'
#     else
#         regex=$regex'(.*)$'
#     fi
#
#     # Repeats the [[ ]] command until it's not 0 any more.
